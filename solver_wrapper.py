...
def run_basic_mapf_with_allocation(
    ...
    cbs_node_budget : max CBS node expansions before declaring the allocation
        infeasible.  Needed because a truly infeasible allocation (e.g. two
        agents whose assignments force them to cross in a width-1 corridor)
        does not make CBS terminate — it accumulates ever-larger time
        constraints forever.  Note the framework uses vanish-at-target
        semantics, so a finished agent does not block others; crossing is the
        minimal infeasible case.  The normal solver escapes such allocations
        via k=2 LKH re-allocation; this path deliberately cannot.

    Returns the same dict shape as run_basic_mapf, or None if the allocation
    is infeasible / budget-exceeded.  "cost" is the true execution cost:
    total collision-free path length under this allocation.
    """
    if config_str is None:
