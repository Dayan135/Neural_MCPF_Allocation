#!/bin/bash
# TIERA extrapolation eval: tierA_maze_s0 on the maze-32-32 map only
# (beyond its own training grid): N∈{190,205,220} × M∈{370,400,430}.
# 9-task array; N/M chosen to stay under the free-cell ceiling.
# Writes per-instance CSVs to results/fullpipe_tierA_extrap/maze/<map>_n{N}m{M}.csv.
#
# Submit after tierA_maze_s0 has finished training:
#   sbatch scripts/eval_tierA_extrap_maze.sh

#SBATCH --job-name=mcpf_etierA_ex_maze
#SBATCH --array=0-8
...
#SBATCH --mem=16G
#SBATCH --output=/home/dayanb/course_multiagent/Neural_MCPF_Allocation/logs/eval_tierA_extrap_maze_%A_%a.out
#SBATCH --error=/home/dayanb/course_multiagent/Neural_MCPF_Allocation/logs/eval_tierA_extrap_maze_%A_%a.err
...
mkdir -p "$PROJECT/results/fullpipe_tierA_extrap/maze"
...
python evaluation/full_pipeline_eval.py \
    ...
    --csv "$PROJECT/results/fullpipe_tierA_extrap/maze/${MAP}_n${N}m${M}.csv"
