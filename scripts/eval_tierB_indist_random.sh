#!/bin/bash
# TIERB in-distribution eval: tierB_random_s0 on the random-32-32 map only
# (its own training grid): N∈{30,55,80} × M∈{50,100,150}.
# 9-task array (this map's 9 configs only).
# Writes per-instance CSVs to results/fullpipe_tierB_indist/random/<map>_n{N}m{M}.csv.
#
# Submit after tierB_random_s0 has finished training:
#   sbatch scripts/eval_tierB_indist_random.sh

#SBATCH --job-name=mcpf_etierB_in_rand
#SBATCH --array=0-8
...
#SBATCH --mem=16G
#SBATCH --output=/home/dayanb/course_multiagent/Neural_MCPF_Allocation/logs/eval_tierB_indist_random_%A_%a.out
#SBATCH --error=/home/dayanb/course_multiagent/Neural_MCPF_Allocation/logs/eval_tierB_indist_random_%A_%a.err
...
mkdir -p "$PROJECT/results/fullpipe_tierB_indist/random"
...
python evaluation/full_pipeline_eval.py \
    ...
    --csv "$PROJECT/results/fullpipe_tierB_indist/random/${MAP}_n${N}m${M}.csv"
