#!/bin/bash
# TIERA map extrapolation eval: tierA_maze_s0 (trained on maze only) on ALL 4 maps
...
# Writes per-instance CSVs to results/fullpipe_tierA_mapextrap/maze/<map>_n{N}m{M}.csv.
#
# Submit after tierA_maze_s0 has finished training:
#   sbatch scripts/eval_tierA_mapextrap_maze.sh

#SBATCH --job-name=mcpf_etierA_mp_maze
#SBATCH --array=0-35
...
#SBATCH --mem=16G
#SBATCH --output=/home/dayanb/course_multiagent/Neural_MCPF_Allocation/logs/eval_tierA_mapextrap_maze_%A_%a.out
#SBATCH --error=/home/dayanb/course_multiagent/Neural_MCPF_Allocation/logs/eval_tierA_mapextrap_maze_%A_%a.err
...
mkdir -p "$PROJECT/results/fullpipe_tierA_mapextrap/maze"
...
python evaluation/full_pipeline_eval.py \
    ...
    --csv "$PROJECT/results/fullpipe_tierA_mapextrap/maze/${MAP}_n${N}m${M}.csv"
