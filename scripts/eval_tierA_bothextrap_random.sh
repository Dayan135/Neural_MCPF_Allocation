#!/bin/bash
# TIERA both extrapolation eval: tierA_random_s0 (trained on random only) on ALL 4 maps
...
# Writes per-instance CSVs to results/fullpipe_tierA_bothextrap/random/<map>_n{N}m{M}.csv.
#
# Submit after tierA_random_s0 has finished training:
#   sbatch scripts/eval_tierA_bothextrap_random.sh

#SBATCH --job-name=mcpf_etierA_bo_rand
#SBATCH --array=0-35
...
#SBATCH --mem=16G
#SBATCH --output=/home/dayanb/course_multiagent/Neural_MCPF_Allocation/logs/eval_tierA_bothextrap_random_%A_%a.out
#SBATCH --error=/home/dayanb/course_multiagent/Neural_MCPF_Allocation/logs/eval_tierA_bothextrap_random_%A_%a.err
...
mkdir -p "$PROJECT/results/fullpipe_tierA_bothextrap/random"
...
python evaluation/full_pipeline_eval.py \
    ...
    --csv "$PROJECT/results/fullpipe_tierA_bothextrap/random/${MAP}_n${N}m${M}.csv"
