#!/bin/bash
# TIERB extrapolation eval: tierB_random_s0 on the random-32-32 map only
# (beyond its own training grid): N∈{60,120,180} × M∈{100,225,350}.
# 9-task array; target = Tier A's own training grid.
# Writes per-instance CSVs to results/fullpipe_tierB_extrap/random/<map>_n{N}m{M}.csv.
#
# Submit after tierB_random_s0 has finished training:
#   sbatch scripts/eval_tierB_extrap_random.sh

#SBATCH --job-name=mcpf_etierB_ex_rand
#SBATCH --array=0-8
...
#SBATCH --mem=16G
#SBATCH --output=/home/dayanb/course_multiagent/Neural_MCPF_Allocation/logs/eval_tierB_extrap_random_%A_%a.out
#SBATCH --error=/home/dayanb/course_multiagent/Neural_MCPF_Allocation/logs/eval_tierB_extrap_random_%A_%a.err
...
mkdir -p "$PROJECT/results/fullpipe_tierB_extrap/random"
...
python evaluation/full_pipeline_eval.py \
    ...
    --csv "$PROJECT/results/fullpipe_tierB_extrap/random/${MAP}_n${N}m${M}.csv"
