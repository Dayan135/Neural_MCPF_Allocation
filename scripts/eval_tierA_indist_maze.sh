#!/bin/bash
# TIERA in-distribution eval: tierA_maze_s0 on the maze-32-32 map only
# (its own training grid): N∈{60,120,180} × M∈{100,225,350}.
# 9-task array (this map's 9 configs only).
# Writes per-instance CSVs to results/fullpipe_tierA_indist/maze/<map>_n{N}m{M}.csv.
#
# Submit after tierA_maze_s0 has finished training:
#   sbatch scripts/eval_tierA_indist_maze.sh

#SBATCH --job-name=mcpf_etierA_in_maze
#SBATCH --array=0-8
...
#SBATCH --mem=16G
#SBATCH --output=/home/dayanb/course_multiagent/Neural_MCPF_Allocation/logs/eval_tierA_indist_maze_%A_%a.out
#SBATCH --error=/home/dayanb/course_multiagent/Neural_MCPF_Allocation/logs/eval_tierA_indist_maze_%A_%a.err
...
mkdir -p "$PROJECT/results/fullpipe_tierA_indist/maze"
...
python evaluation/full_pipeline_eval.py \
    ...
    --csv "$PROJECT/results/fullpipe_tierA_indist/maze/${MAP}_n${N}m${M}.csv"
