#!/bin/bash
# Full-pipeline eval: paper_current_s0 (h128/L6) on all 36 paper_maps configs.
# 36-task array — one per (map, N, M). Writes per-instance CSVs to
# results/fullpipe_paper/current/<map>_n{N}m{M}.csv for later aggregation.
#
# Submit both eval scripts in parallel:
#   sbatch scripts/eval_paper_maps_current.sh
#   sbatch scripts/eval_paper_maps_larger.sh

#SBATCH --job-name=mcpf_eval_current
#SBATCH --array=0-35
...
#SBATCH --error=/home/dayanb/course_multiagent/Neural_MCPF_Allocation/logs/eval_current_%A_%a.err
...
mkdir -p "$PROJECT/results/fullpipe_paper/current"
