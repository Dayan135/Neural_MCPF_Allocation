#!/bin/bash
# TIERB extrapolation eval: tierB_maze_s0 on the maze-32-32 map only
# (beyond its own training grid): N∈{60,120,180} × M∈{100,225,350}.
# 9-task array; target = Tier A's own training grid.
# Writes per-instance CSVs to results/fullpipe_tierB_extrap/maze/<map>_n{N}m{M}.csv.
#
# Submit after tierB_maze_s0 has finished training:
#   sbatch scripts/eval_tierB_extrap_maze.sh

#SBATCH --job-name=mcpf_etierB_ex_maze
#SBATCH --array=0-8
...
#SBATCH --mem=16G
#SBATCH --output=/home/dayanb/course_multiagent/Neural_MCPF_Allocation/logs/eval_tierB_extrap_maze_%A_%a.out
#SBATCH --error=/home/dayanb/course_multiagent/Neural_MCPF_Allocation/logs/eval_tierB_extrap_maze_%A_%a.err
...
mkdir -p "$PROJECT/results/fullpipe_tierB_extrap/maze"
...
python evaluation/full_pipeline_eval.py \
    ...
    --csv "$PROJECT/results/fullpipe_tierB_extrap/maze/${MAP}_n${N}m${M}.csv"
