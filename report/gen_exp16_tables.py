"""
Exp 16 report tables/figures — Tier A (dataset-B leg) only.

Tier B (dataset A) is still running on the cluster (see report/data/exp16/manifest.md), so this
script only builds the Stage-2 (specialist-vs-generalist, R3/R4) and scale-extrapolation pieces
that Tier A alone supports. Stage 1 (dataset-A-vs-B, R2) needs both tiers and isn't attempted here.
...
ax.set_title("Tier A (dataset B) — specialist vs. generalist,\nexecution-cost ratio, own N,M grid",
             fontsize=11.5)
...
with open(tex_path, "w") as f:
    ...
    f.write("\\bottomrule\n")
    f.write("\\caption{Full per-configuration results, dataset-B leg (own $N,M$ grid). "
            "174 of 180 possible (model, map, $N$, $M$) cells; missing cells were not yet "
            "finished on the cluster at pull time.}\n")
    f.write("\\label{tab:fullconfig}\n\\end{longtable}\n}\n")
print(f"Wrote {tex_path}")

print("\n--- headline numbers (own N,M grid; Tier A / dataset B only) ---")
