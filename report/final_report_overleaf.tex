...
\hl{NOTE: all the Contributions should be explained better.
Its too general}
...
\hl{NOTE: Explain what is the matrix sizes, what the columns and rows represent}
...
\hl{ADD: a concrete numeric example (little). Can be graphical.}
...
through the labels $Y$. \hl{ADD: an explanatin why didn't we use the real sum of costs (after allocation). claim: it would have taken to much time for training.}
...
\textbf{D3. One size-agnostic model beats a family of specialists.} \hl{reframe the D3 title, specialist is used later and can be confusin} A universal model trained
...
\textbf{Two training regimes, one per tier.} We compare two datasets, and deliberately name them to
match the model tier trained on each, so the two naming schemes never disagree. \textbf{Dataset A}
is the \emph{large} regime: $N\in\{60,120,180\}$, $M\in\{100,225,350\}$, 3000 train / 500 val / 500
test instances per configuration, labeled at a per-instance solver time that on the hardest cells
approaches the practical one-minute ceiling above. \textbf{Dataset B} is the \emph{regular} regime:
$N\in\{30,55,80\}$, $M\in\{50,100,150\}$, 20000/2000/2000 instances per configuration, labeled in
roughly ten seconds per instance, $6\times$ more samples than Dataset A per configuration. Five
roles, one generalist trained on all four maps and four specialists trained on \texttt{empty},
\texttt{random}, \texttt{maze} and \texttt{room} respectively, are each trained once on Dataset A,
giving the five \textbf{Tier A} models (Table~\ref{tab:modelinv}), and once on Dataset B, giving
the five \textbf{Tier B} models (Table~\ref{tab:modelinvB}). Ten models in total, all h128/L6.
...
extrapolates better, but Dataset A places the large-instance regime inside the training distribution
...
\begin{table}[htbp]
\centering
\caption{Model inventory, Dataset A leg (Tier A, five roles).}
\small
...
\begin{table}[htbp]
\centering
\caption{Model inventory, Dataset B leg (Tier B, five roles).}
\small
\begin{tabularx}{\textwidth}{llXcccc}
\toprule
Model & Role & Train map(s) & $N$ range & $M$ range & Epochs & Batch \\
...
evaluated throughout. Tier A trained 3 seeds per role (0--2); Tier B trained 2 (0--1, cut from a
planned 3 partway through to let all four per-map configs run in parallel under the cluster's
per-user GPU quota). Tier A's batch size (4) is far smaller than Tier B's (32) because Tier A's
instances are larger ($N,M$ up to $180\times350$) and hit GPU memory limits sooner. Best
validation loss, best epoch and training wall-time are blank for Tier A (only evaluation CSVs
were pulled, not per-epoch logs) and partly blank for Tier B (available for the four specialists,
...
and 0\% at larger ones, which is exactly the profile that motivated Dataset A's
...
Figure~\ref{fig:stage1crossover} answers RQ2 by evaluating each role's Dataset A and Dataset B
checkpoints on a common pair of grids spanning both regimes, so every checkpoint is scored both at
home and away. The left panel is Dataset B's native grid (small, $N\in\{30,55,80\}$,
$M\in\{50,100,150\}$): Dataset B is interpolating inside its training range there, while Dataset A
is extrapolating downward, to sizes well below anything it ever trained on. The right panel is
Dataset A's native grid (large, $N\in\{60,120,180\}$, $M\in\{100,225,350\}$): the roles reverse,
Dataset A is at home and Dataset B is extrapolating upward, to sizes beyond anything it ever
trained on.
...
lines never cross: Dataset B (green) sits below Dataset A (navy) at every point in both panels,
whether Dataset B is interpolating at home (left) or extrapolating up (right).}
\label{fig:stage1crossover}
\end{figure}

\input{stage1_winner_table.tex}

Table~\ref{tab:stage1winners} confirms it numerically: no crossover for any role. Dataset B wins
on both its own small grid and on Dataset A's large grid, for all five roles, everywhere it was
tested — including the right panel, where Dataset B is the one extrapolating and Dataset A is on
...
its own regime. The likely explanation is sample count: Dataset B supplies $6\times$ more
instances per configuration (20k/2k/2k against 3k/500/500), and that advantage evidently
outweighs training directly at the target scale, even when Dataset A is evaluated on its own home
turf. For a fixed labeling budget the result favors many regular-size instances over fewer large
ones: Dataset B's checkpoints extrapolate to the large regime better than Dataset A's checkpoints
perform natively there, at a fraction of the per-instance solver cost needed to generate the
labels in the first place.

\begin{table}[htbp]
\centering
\caption{Joint (generalist) versus the native specialist, Dataset A leg (Tier A models), own
...
\caption{Full $5\times4$ matrix, Dataset A leg (Tier A models): every model (rows) on every map
...
On RQ2, we expect Dataset B to win at small $(N,M)$ and Dataset A at large $(N,M)$, with a crossover
...
\textbf{Verdict, Dataset A leg (RQ3).} The RQ3 prediction was half right. We expected the
...
from a mixed result: we expected a crossover, and found none. Dataset B wins for every role on
...
On the Dataset A leg (Tier A models), map specialists do not beat the generalist. The two tie on
...
the better use of a fixed labeling budget is now answered: Dataset B (regular size, more samples)
beats Dataset A (large size, fewer samples) outright, on its own grid and extrapolated onto
Dataset A's, for every one of the five roles, with no crossover.
...
\subsection*{Full per-configuration results, Dataset A leg (Tier A)}
...
$5\times10^{-4}$, gradient clip 1.0, 50-epoch budget, seed 0 evaluated. Tier A (Dataset A) used
batch size 4 and 3 seeds trained (0--2); Tier B (Dataset B) used batch size 32 and 2 seeds trained
(0--1, cut from a planned 3 partway through to let all four per-map configs run in parallel under
the cluster's per-user GPU quota) — Tier A's larger instances ($N,M$ up to $180\times350$) hit GPU
memory limits at a much smaller batch size than Tier B's. Training ran on the cluster's
...
\caption{Training convergence for Tier B (Dataset B)'s four specialists — the only 4 of the 10
...
identifiers) — but in its \emph{own} naming, where ``Tier A'' pairs with ``dataset B'' and
``Tier B'' with ``dataset A'', the reverse of this report's Dataset A/Dataset B convention, which
we chose instead to match each tier's own letter. Translate by swapping the letter when
cross-referencing raw file paths against the manifest.
