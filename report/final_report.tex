...
The architecture therefore places no upper bound on $N$ or $M$. The largest instances in the design
study, 50 agents and 100 goals, are limited by the exact solver instead: every result is a ratio
against its output, so the baseline has to finish before a comparison exists at all.
Section~\ref{sec:final} pushes this further, to 220 agents and 430 goals on the real benchmark
maps, and measures where the ceiling actually falls.
...
\begin{table}[htbp]
\centering
\caption{Model inventory, dataset-B leg (five roles). Dataset A has since finished training and
is evaluated below; its five rows are delivered in
\texttt{report/data/exp16/model\_inventory.csv} rather than duplicated in this table.}
\small
...
\smallskip
\small\noindent All five share the h128/L6 universal architecture, about 1.19M parameters, the
same as Exp 12's h128/L6, with lr $5\times10^{-4}$ and gradient clipping 1.0. Three seeds were
trained per role (0--2); seed 0 is the one evaluated below. Best validation loss, best epoch and
training wall-time are not shown because only the evaluation CSVs were pulled from the cluster for
these runs, not the per-epoch training logs. Full columns in
...
and 0\% at larger ones, which is exactly the profile that motivated dataset B's
...
\caption{Dataset A vs.\ B, mean cost ratio over each role's own domain, on both regimes. Solid
lines never cross: dataset A (green) sits below dataset B (navy) at every point in both panels.}
\label{fig:stage1crossover}
\end{figure}

\input{stage1_winner_table.tex}

Table~\ref{tab:stage1winners} and Figure~\ref{fig:stage1crossover} show no crossover for any
role: dataset A wins on both its own small grid and on dataset B's large grid, for all five
roles, everywhere it was tested. The margin ranges from $0.011$ on \texttt{maze} to $0.027$ on
\texttt{empty}, and the two curves rise together with $M$ rather than swapping order, so the gap
is consistent rather than regime-dependent. This contradicts the RQ2 hypothesis, which expected
each dataset to win in its own regime. The likely explanation is sample count: dataset A supplies
$6\times$ more instances per configuration (20k/2k/2k against 3k/500/500), and that advantage
evidently outweighs training directly at the target scale, even when dataset B is evaluated on
its own home turf. For a fixed labeling budget the result favors many regular-size instances over
fewer large ones: dataset A's checkpoints extrapolate to the large regime better than dataset B's
checkpoints perform natively there, at a fraction of the per-instance solver cost needed to
generate the labels in the first place.

\begin{table}[htbp]
\centering
\caption{Joint (generalist) versus the native specialist, dataset-B leg, own $N,M$ grid. Speedup is
mean solver wall-time divided by mean NN pipeline wall-time, same node. $n$ is instances evaluated,
joint/specialist.}
\small
...
\caption{Full $5\times4$ matrix, dataset-B leg: every model (rows) on every map (columns), execution-cost
ratio, own $N,M$ grid. Asterisked cells are a specialist on its own map. A heatmap is used in place
of grouped bars to keep all 20 cells, diagonal and off-diagonal, legible in one figure.}
\label{fig:tierA54}
\end{figure}
...
\textbf{Verdict, dataset-B leg.} The RQ3 prediction was half right. We expected the specialist
advantage to be largest on maze. Instead the two tie everywhere, including maze, 1.057 against
1.057. The structure-specific effect D5 predicted is real but sits off-diagonal, not on it. It is
misrouting to maze, not training on maze, that is expensive, which is the more interesting outcome
we flagged as the alternative. The timing prediction failed outright: specialist and joint are not
indistinguishable, speedup ranges from 2.3$\times$ for a specialist misrouted to maze to
17.1$\times$ for a specialist on its own map, and the gap tracks fallback replanning rather than
the environment. The RQ2 prediction failed as well, in the opposite direction from a mixed
result: we expected a crossover, and found none. Dataset A wins for every role on both regimes,
so the more-data, moderate-scale strategy is the better use of a fixed labeling budget outright,
not merely in its own range.
...
On the dataset-B leg, map specialists do not beat the generalist. The two tie on cost everywhere,
including maze, and the generalist wins on wall-time for one of the four maps. What does justify
routing is avoiding the alternative failure mode: sending an instance to the wrong specialist costs
up to 52\% on maze specifically, so a router is worth building only if it can identify the map
reliably, not to chase a specialist-vs-generalist quality gap that this leg did not find. Which
training distribution, many regular-size instances or fewer large ones, makes the better use of a
fixed labeling budget is not yet answered: it needs dataset A, still training on the cluster at the
time of writing.
...
\begin{TODO}{TEXT --- decide before submission}
Related work (Section~2.4) is deliberately one paragraph; expand it into a standalone section if
the grader expects a fuller treatment. (References below have been checked against their
original venues, years and page numbers; two corrections were applied: \texttt{robustmcpf} was
missing its AAAI volume/issue/pages, and \texttt{pointernet} cited the retroactive name
``NeurIPS'' for a paper published in 2015 under the conference's contemporaneous name, NIPS.)
\end{TODO}
...
\subsection*{Full per-configuration results, dataset-B leg}

\input{tierA_fullconfig_table.tex}

\subsection*{Reproducibility, dataset-B leg}

All five models: h128/L6 universal transformer, \texttt{use\_goal\_dists} enabled, lr
$5\times10^{-4}$, gradient clip 1.0, batch size 4, 50-epoch budget, 3 seeds trained (0--2), seed 0
evaluated. Training ran on the cluster's \texttt{gpu} partition, \texttt{rtx\_6000} GPUs. Evaluation
(\texttt{full\_pipeline\_eval.py}, both the NN pipeline and the solver baseline) ran on the
\texttt{cpu} partition, same node per cell. Torch version and exact Slurm job identifiers were not
recorded on the machine this report is written on and are not yet available.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.92\textwidth]{fig_tierB_convergence.png}
\caption{Training convergence for Tier B (dataset A)'s four specialists — the only 4 of the 10
...
here the way it did in Section~\ref{sec:design}. Tier A's logs and Tier B's joint model were not
...
\texttt{report/data/exp16/manifest.md} documents the Tier A/dataset-B naming, what is and is not
included, and the open items: dataset A, offline accuracy, training logs, and cluster job
identifiers.
