...
problem, and it is the combinatorial part of an otherwise tractable pipeline. The exact RobustMCPF
solver handles it with an LKH TSP call, then passes a fixed allocation to conflict-based search
(CBS) for collision-free planning. We replace only the allocation step with a learned,
size-agnostic transformer. It reads two shortest-path distance matrices, one from agents to goals
and one between goals, and outputs a distribution over agents for each goal. Since CBS is shared by
both pipelines, any difference in the final plan comes from the allocator alone, and a single set
of weights serves any problem size from $2\times2$ to $50\times100$. Across 28 configurations the
learned allocator stays within 1--6\% of the exact solver's true execution cost and produces no
infeasible allocations. At paper scale it runs 5.9--9.4$\times$ faster while remaining within
...
breadth-first-search distance matrices: $D$ from agents to goals, and $G$ between goals. The
downstream ordering and CBS steps are left untouched, so both pipelines end in the same planner and
any difference between them can be attributed to the allocator. The trade is exactness for speed.
...
\begin{itemize}
  ...
  \item A characterization of what transfers: map geometry does, problem scale does not
...
once, while inference is a forward pass whose cost is close to flat in problem size. What is given
up is quality, so the report's job is to measure that loss in the currency that matters, namely the
length of the collision-free plan actually executed, and to identify where the exchange pays off.
...
Both pipelines share everything except the allocator (Figure~\ref{fig:pipelines}). The solver runs
LKH followed by CBS. Ours runs a forward pass, then goal-visit ordering, then CBS. Because the
planner, the instance and the cost measure are identical, the difference between the two output
plans isolates the allocation decision.
...
M}$ between goals (Figure~\ref{fig:dg}). Both are normalized to $[0,1]$ by $(W-1)+(H-1)$, the
maximum BFS distance on an open $W\times H$ grid.
...
\textbf{Goal context.} Each scalar $G_{jk}$ is embedded and the results are summed over $k$, giving
one context vector per goal $j$ that is added to that goal's column of tokens. The sum-pooling step
is what matters here: it maps a variable-length set of goal-to-goal distances onto a fixed-width
vector, so $M$ can be anything.
...
\textbf{Mixed-size training.} Instances of different shapes cannot be stacked into one tensor, so a
...
At deployment the arg-max allocation is decoded, each agent's visit order is fixed by brute-force
enumeration over its assigned goals, and CBS plans the paths. When an allocation admits no
...
Section~\ref{sec:method}. Table~\ref{tab:decisions} collects the supporting numbers.
...
adjacent and should be bundled. $G$ supplies precisely that, and it outweighs architecture, data
and capacity combined by more than tenfold. This fixes the input.
...
\textbf{Limitations.} Goal-visit ordering is still brute-force over each agent's assigned goals,
which is $O(k!)$ and becomes the pipeline bottleneck at seven or eight goals per agent. The
allocation itself is sub-millisecond, but the classical step around it is not. CBS execution cost is
...
closely enough that the resulting collision-free plans cost 1--6\% more than optimal
