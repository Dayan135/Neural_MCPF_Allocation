...
9.6\% of optimal at 6.6--16.1$\times$ speedup.
\TODOin{closing sentence with the specialist-vs-generalist headline, once dataset A lands}
\end{abstract}
...
\begin{table}[htbp]
\centering
\caption{Model inventory, dataset-B leg (five roles). Dataset A is still training on the cluster;
five more rows are added once it finishes.}
\small
...
the environment. RQ2 cannot be scored yet. Dataset A's training run had not finished when this
section was written, so the dataset-A-vs-B crossover is still open.
