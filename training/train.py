"""
Training loop for GoalAllocMLP (mTSP allocation).

Usage:
    python train.py [--data_dir DIR] [--N 2] [--M 2] [--hidden 64]
...
def per_goal_accuracy(P: torch.Tensor, Y: torch.Tensor) -> float:
    """
    For each goal j, did we predict the correct agent?
    pred[b, j] = argmax over agents of P[b, :, j]
    true[b, j] = argmax over agents of Y[b, :, j]
    """
...
def main():
    ...
    print(f"Device: {device}  |  N={args.N}, M={args.M}, hidden={args.hidden}, lam={args.lam}")
    ...
    model = GoalAllocMLP(N=args.N, M=args.M, hidden=args.hidden).to(device)
